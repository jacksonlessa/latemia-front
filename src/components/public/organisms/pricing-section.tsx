...
interface PricingSectionProps {
  ...
  /**
   * Optional price override (cents) from the live backend setting. When
   * provided, takes precedence over `data.priceCents` — keeps the v1
   * landing in sync with `/admin/configuracoes`. When omitted, falls back
   * to the static value from `landingContent`.
   */
  priceCentsOverride?: number;
}

export function PricingSection({ data, priceCentsOverride }: PricingSectionProps) {
  const effectiveCents = priceCentsOverride ?? data.priceCents;
  return (
    <section className="px-6 md:px-10 py-16 max-w-3xl mx-auto text-center">
      ...
      <div className="flex flex-col items-center gap-2 mb-4">
        <PriceTag cents={effectiveCents} className="font-display text-5xl md:text-6xl text-forest" />
