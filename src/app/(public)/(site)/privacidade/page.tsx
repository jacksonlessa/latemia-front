import type { Metadata } from 'next';
import { getSeoMetadata } from '@/config/seo';
import { MarkdownProse } from '@/components/ui/markdown-prose';
import {
  POLITICA_PRIVACIDADE_TEXTO,
  PRIVACY_POLICY_VERSION,
  PRIVACY_POLICY_EFFECTIVE_DATE,
} from '@/content/privacidade';

export const metadata: Metadata = getSeoMetadata('/privacidade');

export default function PrivacidadePage() {
  return (
    <main className="px-6 md:px-10 py-16 max-w-3xl mx-auto">
      <p className="text-xs uppercase tracking-wide text-forest/60 font-medium mb-2">
        Versão {PRIVACY_POLICY_VERSION} — em vigor desde{' '}
        {new Date(PRIVACY_POLICY_EFFECTIVE_DATE).toLocaleDateString('pt-BR')}
      </p>

      <MarkdownProse>{POLITICA_PRIVACIDADE_TEXTO}</MarkdownProse>
    </main>
  );
}
